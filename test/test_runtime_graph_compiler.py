...
from methods.runtime_evaluator.compiler_model import RuntimeObservedCompileError
...
class RuntimeGraphCompilerTestCase(unittest.TestCase):
    ...
    def test_runtime_compiler_preserves_assignment_prefixed_nonzero_source_status(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "VAR=foo source ./dep.sh\n"
    ...
    def test_runtime_compiler_preserves_handled_assignment_prefixed_nonzero_source_status(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "VAR=foo source ./dep.sh || printf 'handled:%s\\n' \"$?\"\n",
            )
            project.write("dep.sh", "printf 'dep:%s\\n' \"$VAR\"\nreturn 7\n")
            expected = project.run("main.sh")
            compiled, _graph = self.compile_observed(project, "main.sh")
    ...
    def test_runtime_compiler_preserves_redirected_assignment_prefixed_nonzero_source_status(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "VAR=foo source ./dep.sh > out.txt\n"
                "printf 'status:%s\\n' \"$?\"\n"
                "cat out.txt\n",
            )
            project.write("dep.sh", "printf 'dep:%s\\n' \"$VAR\"\nreturn 7\n")
    ...
    def test_runtime_compiler_preserves_negated_assignment_prefixed_source(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "! VAR=foo source ./dep.sh\n"
                "printf 'status:%s var:%s\\n' \"$?\" \"${VAR:-}\"\n",
            )
            project.write("dep.sh", "printf 'dep:%s\\n' \"$VAR\"\nreturn 7\n")
            expected = project.run("main.sh")
            compiled, _graph = self.compile_observed(project, "main.sh")
    ...
    def test_runtime_compiler_preserves_negated_assignment_prefixed_source_condition(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "if ! VAR=foo source ./dep.sh; then\n"
                "  printf 'then:%s\\n' \"${VAR:-}\"\n"
                "else\n"
                "  printf 'else:%s\\n' \"${VAR:-}\"\n"
                "fi\n",
    ...
    def test_runtime_compiler_preserves_backslash_continued_source(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "source \\\n"
    ...
    def test_runtime_compiler_preserves_backslash_continued_assignment_source(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "VAR=foo source \\\n"
    ...
    def test_runtime_compiler_preserves_backslash_continued_source_redirection(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "source \\\n"
    ...
    def test_runtime_compiler_preserves_continued_source_with_trailing_command(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "source \\\n"
    ...
    def test_runtime_compiler_preserves_continued_sources_with_surrounding_commands(self):
        cases = {
            "suffix and": (
                "source \\\n"
                "  ./dep.sh && printf 'ok\\n'\n"
    ...
    def test_runtime_compiler_preserves_process_substitution_source_arguments(self):
        cases = {
            "direct": (
                "source ./dep.sh <(printf 'ARGDATA\\n')\n",
                "printf 'argpath:%s\\n' \"$1\"\n"
                "cat \"$1\"\n"
                "printf 'catstatus:%s\\n' \"$?\"\n",
            ),
            "assignment": (
                "VAR=foo source ./dep.sh <(printf 'ARGDATA\\n')\n",
                "printf 'var:%s argpath:%s\\n' \"$VAR\" \"$1\"\n"
                "cat \"$1\"\n"
                "printf 'catstatus:%s\\n' \"$?\"\n",
            ),
            "child bash": (
                "bash -c 'source ./dep.sh <(printf \"ARGDATA\\n\")'\n",
                "printf 'argpath:%s\\n' \"$1\"\n"
                "cat \"$1\"\n"
                "printf 'catstatus:%s\\n' \"$?\"\n",
            ),
    ...
    def test_runtime_compiler_preserves_dynamic_command_process_substitution_redirection(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "cmd=cat\n"
    ...
    def test_runtime_compiler_preserves_dynamic_command_process_substitution_redirection_in_child_bash(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "bash -c 'source ./dep.sh; cmd=cat; \"$cmd\" < <(printf \"psub\\\\n\"); printf \"after:%s\\\\n\" \"$?\"'\n",
    ...
    def test_runtime_compiler_preserves_middle_source_in_child_bash_payload(self):
        with ScriptProject() as project:
            project.write("main.sh", "bash -c 'echo pre; source ./dep.sh; echo post'\n")
            project.write("dep.sh", "printf 'dep\\n'\n")
            expected = project.run("main.sh")
            compiled, _graph = self.compile_observed(project, "main.sh")
            actual = project.run(compiled)
    ...
    def test_runtime_compiler_preserves_middle_source_in_child_bash_payload_with_argv(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "bash -c 'echo pre; source ./dep.sh; echo \"$0:$1\"; echo post' child0 arg1\n",
            )
            project.write("dep.sh", "printf 'dep\\n'\n")
            expected = project.run("main.sh")
            compiled, _graph = self.compile_observed(project, "main.sh")
    ...
    def test_runtime_compiler_replays_option_terminator_no_argument_source_failure(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "source -- 2>err.txt || true\n"
    ...
    def test_runtime_compiler_invalid_source_option_does_not_source_same_named_file(self):
        with ScriptProject() as project:
            project.write("-p", "SIDE=mutated\nreturn 2\n")
            project.write(
                "main.sh",
                "source -p || true\n"
                "printf 'after:%s\\n' \"${SIDE:-none}\"\n",
        ...
        self.assertEqual(graph["edges"][0]["failure_kind"], "invalid-option")
        self.assertEqual(result.returncode, expected.returncode, result.stdout)
        self.assertEqual(result.stdout, expected.stdout)
        self.assertNotIn("after:mutated", result.stdout)
    ...
    def test_runtime_compiler_preserves_dynamic_leading_source_redirections(self):
        cases = {
            "separate-output": (
                '> "$(false; echo out.txt)" source ./dep.sh\ncat out.txt\n',
                "printf 'inside:%s\\n' \"$?\"\n",
            ),
            "combined-output": (
                '>"$(false; echo out.txt)" source ./dep.sh\ncat out.txt\n',
            ...
            "input": (
                '< "$(printf input.txt)" source ./dep.sh\n',
                "read value\nprintf 'inside:%s\\n' \"$value\"\n",
            ),
        }
        for name, (script, dep) in cases.items():
    ...
    def test_runtime_compiler_replays_nested_wrapper_sources(self):
        cases = {
    ...
    def test_runtime_compiler_replays_same_line_elif_brace_condition_source_span(self):
        cases = {
            "source-only": (
                "if false; then :; elif { source ./dep.sh; }; then echo then; fi\n",
                "dep\nthen\n",
            ),
            "source-and-tail": (
                "if false; then :; elif { source ./dep.sh; printf after; }; then echo then; fi\n",
                "dep\nafterthen\n",
            ),
        }
        for name, (script, expected_stdout) in cases.items():
            with self.subTest(name=name), ScriptProject() as project:
                project.write("dep.sh", "printf 'dep\\n'\n")
                project.write("main.sh", script)
                expected = project.run("main.sh")
    ...
    def test_runtime_compiler_allows_source_free_dynamic_external_c_payloads(self):
        cases = {
            "echo": (
                "cmd=echo\n"
                "PAYLOAD=hello\n"
        ...
        for name, (script, env, expected_stdout) in cases.items():
            with self.subTest(name=name), ScriptProject() as project:
                project.write("main.sh", script)
                project.write("dep.sh", "printf 'dep\\n'\n")
                compiled, _graph = self.compile_observed(project, "main.sh", env=env)
    ...
    def test_runtime_compiler_rejects_dynamic_shell_source_payload_before_output(self):
        with ScriptProject() as project:
            project.write(
                "main.sh",
                "cmd=sh\n"
